...
// Dashboard logic (profile/health/ICE), 6-char codes, reliable QR, working logout.
...
(function () {
  ...
  const CODE_RX    = /^[A-HJ-NP-Z2-9]{6}$/;
  const normalizeDashes = s => (s||'').replace(/[\u2010-\u2015\u2212]/g,'-').toUpperCase(); // just in case

  function makeCode_6(){ return Array.from({length:6},()=>CODE_CHARS[Math.floor(Math.random()*CODE_CHARS.length)]).join(''); }

  function getCleanStoredCode(){
    let code = (localStorage.getItem('myqer_shortcode') || '').toUpperCase().replace(/[^A-HJ-NP-Z2-9]/g,'');
    if (!CODE_RX.test(code)) { code = makeCode_6(); localStorage.setItem('myqer_shortcode', code); }
    return code;
  }
    ...
    return getUserId().then(uid=>{
      ...
      const tryUpsert = () => {
        ...
        return supabase.from('profiles').upsert({ user_id: uid, code }, { onConflict: 'user_id' }).then(res=>{
          ...
          if ((msg.includes('duplicate') || msg.includes('unique')) && attempts < 6){
            code = makeCode_6();
  ...
  /* --------------------- simple embedded QR fallback ------------------- */
  const simpleQR = (function(){ // tiny encoder → draws crisp SVG to canvas
    ...
    getMaxDataBits:function(){return 128*8-4}, // enough for our URLs
    ...
    createData:function(){const buffer=new QRBitBuffer(); for(const d of this.dataList){buffer.put(4,4);buffer.put(d.getLength(),8);d.write(buffer)}
      const maxBits=128*8; if(buffer.length+4<=maxBits)buffer.put(0,4); while(buffer.length%8!==0)buffer.put(0,1);
    ...
    applyBestMask:function(){const scores=[]; for(let m=0;m<4;m++){const cp=this.modules.map(r=>r.slice());
        ...
        scores.push({m,cp,score:0}); // simplified scoring
      }
      scores.sort((a,b)=>a.score-b.score); this.modules = scores[0].cp;
    }};
    function svg(text,size=300,margin=4){
      const q=new QRCode(0); q.addData(text||''); q.make();
      const m=q.modules, n=m.length, scale=Math.max(1, Math.floor(size/(n+2*margin))), dim=scale*(n+2*margin);
    ...
    function canvas(canvas,text,size=300,margin=4){
      const out=svg(text,size,margin), img=new Image(); const url='data:image/svg+xml;charset=utf-8,'+encodeURIComponent(out.svg);
  ...
  function buildOfflineText(shortUrl) {
    ...
    const L = [];
    const L1 = []; if (pf.full_name||pf.fullName) L1.push('Name: '+(pf.full_name||pf.fullName));
    if (pf.date_of_birth||pf.dob) L1.push('DOB: '+(pf.date_of_birth||pf.dob));
    if (pf.country) L1.push('C: '+pf.country);
    if (pf.national_id||pf.healthId) L1.push('ID: '+(pf.national_id||pf.healthId));
  ...
  async function generateQRCode() {
    ...
    try {
      ...
      // Force apex domain to avoid www/apex mismatch on iOS
      ...
      if (window.QRCode && typeof QRCode.toCanvas === 'function') {
        ...
        await simpleQR.canvas(qrCanvas, shortUrl, 300, 4);
      }
      ...
      `<div class="ice-contact-header">
       ...
       <div class="ice-form-grid">
         ...
         <div class="form-group"><label>Phone</label>
           <input type="tel" class="icePhone" data-field="phone" data-idx="${idx}" value="${c.phone||''}" placeholder="+44 7300 000000">
         </div>
       </div>`;
      box.appendChild(row);
    });
  ...
  function saveProfile(){
    ...
    supabase.auth.getSession().then(r=>{
      if (!r?.data?.session){ toast('Saved locally — please sign in to sync','info'); generateQRCode(); return; }
  ...
  function saveHealth() {
    ...
    supabase.auth.getSession()
      .then((r) => {
        if (!r?.data?.session) { calculateTriage(); toast('Saved locally — please sign in to sync','info'); generateQRCode(); throw new Error('no session'); }
        return getUserId();
      })
      .then((uid) => {
  ...
  function fillFromLocal(){
    try{
      ...
      set('hfBloodType',h.bloodType); set('hfAllergies',h.allergies); set('hfConditions',h.conditions); set('hfMeds',h.medications); set('hfImplants',h.implants);
    ...
    return withTimeout(supabase.auth.getSession(),3000,'getSession').then(res=>{
      ...
      return withTimeout(getUserId(),3000,'getUserId').then(uid=>{
        ...
        return withTimeout(supabase.from('profiles').select('*').eq('user_id',uid).maybeSingle(),4000,'profiles.select').then(rp=>{
          ...
          return withTimeout(supabase.from('health_data').select('*').eq('user_id', uid).maybeSingle(),4000,'health.select')
          .then(rh=>{
            ...
            userData.health=Object.assign({},userData.health,h);
          ...
          return withTimeout(supabase.from('ice_contacts').select('*').eq('user_id',uid).order('contact_order',{ascending:true}),4000,'ice.select').then(ri=>{
            const ice=ri?.data||[];
            if (Array.isArray(ice)){ iceContacts=ice.map(r=>({name:r.name||'',relationship:r.relationship||'',phone:r.phone||''})); localStorage.setItem('myqer_ice', JSON.stringify(iceContacts)); renderIceContacts(); }
          });
        });
      });
